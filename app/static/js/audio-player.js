...
/**
 * Audio player and recorder functionality
 */
...
export async function startAudioPlayerWorklet() {
  ...
  const node = new AudioWorkletNode(audioContext, 'audio-player-processor');
  node.connect(audioContext.destination);
  return [node, audioContext];
}

// Start audio recorder worklet
export async function startAudioRecorderWorklet(callback) {
  const audioContext = new AudioContext();
  await audioContext.audioWorklet.addModule('audio-worklet.js');
